  ...
  <div class="bg-white p-4 rounded-lg shadow animate-fade-in-down">
    ...
    <table class="table-auto w-full">
      ...
      <tr>        
        ...
        <TableHeaderCell field="role" :sort-field="sortField" :sort-direction="sortDirection"
      ...
      <tr v-for="(user, index) of users.data">
        ...
        <td class="border-b p-2 max-w-[200px] whitespace-nowrap overflow-hidden text-ellipsis">
          {{ user.role == 'pos' ? 'taquilla' : user.role }}
        </td>       
        <td class="border-b p-2 ">
              ...
              >
                <div class="px-1 py-1">
                  <MenuItem v-slot="{ active }">
                    <router-link :to="{name: 'app.users.show', params: {id: user.id}}"
                        :class="[
                  ...
                  <MenuItem v-slot="{ active }" v-if="store.state.user.data.id != user.id">
                    <button
                      :class="[
                        active ? 'bg-red-600 text-white' : 'text-gray-900',
                        'group flex w-full items-center rounded-md px-2 py-2 text-sm',
    ...
    <div v-if="!users.loading" class="flex justify-between items-center mt-5">
      <div v-if="users.data.length">
        Mostrando desde el {{ users.from }} hasta el {{ users.to }}
      </div>
      <nav
...
import {DotsVerticalIcon, PencilIcon, TrashIcon, EyeIcon} from '@heroicons/vue/outline'
